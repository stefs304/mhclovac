...
def train_binding_model(peptide_list, ic50_list, mhc_name):
    try:
        model = load_model(mhc_name)
    except:
        model = create_model_template(mhc_name)
    binding_model = BindingModel()
    binding_model.fit(peptide_list=peptide_list, ic50_list=ic50_list)
...
def train_ligand_model(peptide_list, label_list, mhc_name):
    pass
