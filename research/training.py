...
for mhc_key in list(data['mhc_allele'].unique()):
    ...
    for fraction in TRAINING_SET_FRACTION_LIST:
        ...
        model = BindingModel(random_state=RANDOM_SEED, n_jobs=-1)
        ...
        print(f'fraction {fraction} score = {r2}')
    ...
    train_binding_model(features=x_train, target_values=y_train, mhc_name=mhc_key, random_state=RANDOM_SEED)

    model = load_model(mhc=mhc_key)
    predictions = model.predict(x_test)
    r2 = round(r2_score(y_test, predictions), 3)
    print(f'r2_score={r2}')
